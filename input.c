...
static void read_keys(void) {
    ...
    if( !BCHK(PD_IDR, 5) )  {      // pwr button NOT pressed
        pwrbut = 0;
    } 
    else {
       pwrbut++;
       if(pwrbut > 600) {   // switch off after 3s
           sim();	    // disable interrupts
	   LCD_BCK0;        // backlit off
           BSET(PD_ODR, 7); // LED off
           BRES(PD_ODR, 6); // PWR off
           IO_IF(D, 6);     // set pwren high z
           IO_IF(D, 5);     // set pwrbut high z
           while(1);        // wait for end
       }
    }
}
